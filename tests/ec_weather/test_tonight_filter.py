...
morning update. After 6 AM local, it should be dropped from the daily
forecast since the night has passed.
"""
...
def _apply_tonight_filter(merged: list[dict], hour: int) -> list[dict]:
    ...
    if merged and merged[0].get("temp_high") is None and hour >= 6:
        return merged[1:]
    return merged
...
class TestTonightFilter:
    ...
    def test_single_tonight_period(self) -> None:
        """Single night-only period results in empty list after 6 AM."""
