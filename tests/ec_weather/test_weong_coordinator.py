...
from ec_weather.transforms import _derive_icon
...
def _build_all_results(
    ...
    """Build a flat all_results list for _aggregate_results.
...
def _make_periods(
    ...
    """Build a minimal periods list for _aggregate_results."""
...
class TestPeriodAggregation:
    """Tests for _aggregate_results business logic on period-level data."""
    ...
    def test_period_pop_is_max_of_timesteps(self, hass: HomeAssistant):
        ...
        output = coord._aggregate_results(all_results, periods, now)
    ...
    def test_period_rain_is_sum_of_timesteps(self, hass: HomeAssistant):
        ...
        # rain_amt layer returns values in meters; ×1000 = mm
    ...
    def test_period_snow_is_sum_of_timesteps(self, hass: HomeAssistant):
        ...
        timesteps = _make_timesteps(utc_start, 3)
        pop_values = [50.0, 50.0, 50.0]
        # snow_amt layer returns values in meters; ×100 = cm
    ...
    def test_no_precip_gives_null_amounts(self, hass: HomeAssistant):
        ...
        output = coord._aggregate_results(all_results, periods, now)
        result = output["periods"][period_key]
        assert result["rain_amt_mm"] is None
        assert result["snow_amt_cm"] is None

    def test_freezing_precip_folds_to_rain_mm(self, hass: HomeAssistant):
        """FreezingPrecipCondAmt folds into rain_amt_mm (×1 since already mm)."""
        coord = _make_weong_coordinator(hass)
        now = datetime(2026, 3, 22, 15, 0, tzinfo=timezone.utc)
        utc_start = datetime(2026, 3, 22, 11, 0, tzinfo=timezone.utc)
        utc_end = datetime(2026, 3, 22, 23, 0, tzinfo=timezone.utc)
        ...
        # FreezingPrecipCondAmt returns mm directly (×1)
        freezing_mm = [2.5, 1.5]
        ...
        assert result["rain_amt_mm"] == 4.0
        # No snow from freezing precip
        assert result["snow_amt_cm"] is None

    def test_ice_pellets_fold_to_snow_cm(self, hass: HomeAssistant):
        """IcePelletsCondAmt folds into snow_amt_cm (×100 m→cm conversion)."""
        coord = _make_weong_coordinator(hass)
        now = datetime(2026, 3, 22, 15, 0, tzinfo=timezone.utc)
        utc_start = datetime(2026, 3, 22, 11, 0, tzinfo=timezone.utc)
        utc_end = datetime(2026, 3, 22, 23, 0, tzinfo=timezone.utc)
        ...
        # IcePelletsCondAmt returns meters; ×100 = cm
...
class TestDeriveIcon:
    ...
    def test_sky_state_clear_day_icon(self):
        ...
        weong = {"sky_state": 1.0}
        icon_code, condition = _derive_icon(weong, hour=14)
        assert icon_code == 0
    ...
    def test_sky_state_clear_night_icon(self):
        ...
        weong = {"sky_state": 1.0}
        icon_code, condition = _derive_icon(weong, hour=22)
        assert icon_code == 30
...
# ---------------------------------------------------------------------------
# _build_periods tests
# ---------------------------------------------------------------------------

class TestBuildPeriods:
    """Tests for _build_periods — day/night period generation with UTC boundaries."""

    def test_day_night_boundaries_utc(self, hass: HomeAssistant):
        """_build_periods creates correct UTC boundaries for local 06:00/18:00.
        ...
        periods = coord._build_periods(today, now_utc)
