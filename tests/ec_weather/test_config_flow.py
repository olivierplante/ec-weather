...
class TestComputeAlertBbox:
    ...
    def test_format_has_one_decimal(self):
        ...
        result = _compute_alert_bbox(45.78, -74.07)
...
class TestComputeGeoMetBbox:
    ...
    def test_format_has_three_decimals(self):
        ...
        result = _compute_geomet_bbox(45.78, -74.07)
        assert result == "44.780,-75.070,46.780,-73.070"
...
class TestAsyncStepUser:
    ...
    async def test_single_match_proceeds_to_confirm(
        ...
        with patch.object(
            ...
            result = await flow.async_step_user(
                user_input={"city_query": "Saint-Jérôme", CONF_LANGUAGE: "en"}
            )

        assert result["step_id"] == "confirm"
        assert flow._selected_city["id"] == "qc-68"
    ...
    async def test_french_language_passed_to_search(
        ...
        flow = _make_flow(hass)
        city = {**SAINT_JEROME_CITY, "name": "Saint-Jérôme"}
        with patch.object(
            ...
            await flow.async_step_user(
                user_input={"city_query": "Saint-Jérôme", CONF_LANGUAGE: "fr"}
            )

        mock_search.assert_called_once_with("Saint-Jérôme", "fr")
...
class TestAsyncStepConfirm:
    ...
    async def test_shows_form_with_no_input(self, hass: HomeAssistant) -> None:
        ...
        assert result["description_placeholders"]["city_name"] == "Saint-Jérôme"
        assert result["description_placeholders"]["province"] == "QC"
        assert result["description_placeholders"]["city_code"] == "qc-68"
    ...
    async def test_creates_entry_with_user_input(
        ...
        user_input = {
            ...
            CONF_GEOMET_BBOX: "44.780,-75.070,46.780,-73.070",
        ...
        assert call_kwargs["title"] == "Saint-Jérôme"
        data = call_kwargs["data"]
        assert data[CONF_CITY_CODE] == "qc-68"
        assert data[CONF_CITY_NAME] == "Saint-Jérôme"
        assert data[CONF_LANGUAGE] == "en"
        assert data[CONF_LAT] == 45.78
        assert data[CONF_LON] == -74.07
        assert data[CONF_BBOX] == "-74.3,45.6,-73.9,46.0"
        assert data[CONF_GEOMET_BBOX] == "44.780,-75.070,46.780,-73.070"
...
class TestRunDiscovery:
    ...
    async def test_discovery_with_valid_coords(
        ...
        mock_aqhi.assert_called_once_with(45.78, -74.07)
...
class TestOptionsFlowInit:
    ...
    async def test_mutable_keys_go_to_options(
        ...
        assert new_data[CONF_CITY_CODE] == "qc-68"
